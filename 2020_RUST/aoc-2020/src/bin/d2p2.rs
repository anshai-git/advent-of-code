...
impl Password {
    fn is_valid(&self) -> bool {
        let match_first: bool = match self.value.chars().nth((self.min - 1) as usize) {
            Some(c) => c == self.target,
            _ => false
        };

        let match_second: bool = match self.value.chars().nth((self.max - 1) as usize) {
            Some(c) => c == self.target,
            _ => false
        };
