use std::{
    fs::File, io::{BufRead, BufReader}, ops::BitXor, process::exit
};
...
fn main() {
    ...
    let regs: (isize, isize, isize) = parse_regs(parts.nth(0));
    let program = parse_program(parts.nth(0));

    println!("\nREG [A]: {}", &regs.0);
    println!("REG [B]: {}", &regs.1);
    println!("REG [C]: {}", &regs.2);

    println!("\nPROGRAM :: {:?}", program);

    let mut computer: Computer = Computer::new();
    computer.load_program(regs, program);
    let output: String = computer.run_program();
