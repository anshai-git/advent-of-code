...
char digit_strings[9][14] = {"one", "two",   "three", "four", "five",
...
uint64_t parse_line(CalibrationDocument *cd, char *raw_line) {
  ...
  while (current_char != '\n') {
    current_char = raw_line[char_index];
    // This could just be a pointer to the original line
    ...
    if (is_alpha(current_char)) {
      for (int i = 0; i < 9 && strlen(raw_line + char_index) >= 3; i++) {
        if (strncmp(raw_line + char_index, digit_strings[i],
                    strlen(digit_strings[i])) == 0) {
          insert_digit(line, (i + 1) + '0');
        }
      }
    }
...
int main(int argc, const char **argv) {
  ...
  // print_calibration_document(&calibration_document);
  printf("RESULT: %ld\n", result);
